  ...
  AccessControlUpgradeable
{
  ...
  event GrantRevoked(
    address indexed beneficiary,
    address indexed vestingProvider,
    uint256 timestamp
  );
   ...
   */
  function initialize(IERC20Upgradeable token_, address pauserAddress_)
    public
    virtual
    initializer
  {
   ...
   */
  function getGrant(address beneficiary_)
    public
    view
    returns (
      uint256 startTime,
    ...
    )
  {
    ...
    beneficiary = _grant.beneficiary;
  ...
  function _addGrant(
    ...
    vestingGrants[beneficiary_] = grant;
  }
  ...
  function addGrant(
    ...
    _addGrant(startTime_, amount_, vestingCliff_, beneficiary_);
    emit GrantAdded(beneficiary_, totalVestedHistory, block.timestamp);
  }
  ...
  function addGrants(
    ...
    require(
      beneficiaries.length > 0 &&
        beneficiaries.length == amounts_.length &&
        startTimes_.length == amounts_.length &&
        startTimes_.length == vestingCliffs_.length,
      "VT10"
    );
   ...
   *
   */
  function revokeGrant(address beneficiary_, address vestingProvider_)
    external
  {
  ...
  function revokeGrants(
    ...
    uint256 i;
    for (i = 0; i < beneficiaries.length; i++) {
      _revokeGrant(beneficiaries[i], vestingProvider_);
    }
